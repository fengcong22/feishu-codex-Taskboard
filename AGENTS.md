...
Bridge 和 Taskboard 只绑定本机 loopback（`127.0.0.1`），不得改成 LAN 或公网监听。Bridge 永不启动 Codex；Taskboard 只有在“本机设置 → 允许本机自动剪辑”显式开启时，才可自动执行由 Bridge 专用接口登记、活动配置快照为 `automatic` 且项目包别名已在本机白名单中的任务。该开关默认关闭；尚未保存本机设置时，兼容 `CODEX_TASKBOARD_ALLOW_AUTOMATIC_EXECUTION` 作为初始值，保存后以本机数据库设置为准。设置立即生效并在重启后保留；关闭只取消尚未开始的自动预约，正在执行的任务继续完成，开启本身不补跑历史任务。普通任务、伪造描述标记或标签不得获得自动执行资格。真实受控执行仅在 Taskboard 确认启动后产生“处理中”回写，并在唯一 ZIP 成功发布后产生阶段完成及首个课程路径回写；目标仅限实时校验后已存在的字段与单选选项，回写通过认证的本机专用接口完成。模拟事件、普通任务和伪造标签不会创建目录、上传 ZIP 或回写飞书；整体投递仍为至少一次，不宣称 exactly-once。
