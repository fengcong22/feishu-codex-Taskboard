...
左侧“本机设置 → 允许本机自动剪辑”提供可点击的总开关，学科配置页也可通过“修改本机总开关”进入。设置立即生效并在本机数据库中保存，重启后保留；默认关闭。只有尚未保存设置时，`CODEX_TASKBOARD_ALLOW_AUTOMATIC_EXECUTION` 才提供初始值，之后界面保存的选择优先（包括关闭）。关闭取消尚未开始的自动延迟或排队预约，排队任务回到待处理，正在执行的剪辑与手动任务继续；重新开启本身不会补跑历史任务。学科选择“自动”不会改变总开关。新任务还必须满足可信 Bridge 登记、已启用活动配置和本机启用包白名单等条件，模拟来源不能自动执行。保存草稿期间原活动配置继续有效，点击启用后由新配置处理后续触发；已有任务的模式和运行状态不会追溯修改，也不会自动重跑。ZIP 与上传中的“上传入队”仍单独控制。
...
升级本机设置与课程标题功能时，先结束活动剪辑/上传、停止服务并冷备份 `.runtime/taskboard`、`.runtime/bridge`、`.env.local` 和本地配置；合入源码后运行 `npm --prefix taskboard run build:web`，再按原监听方式启动服务。真实监听使用 `scripts/start-local.ps1 -EnableFeishu` 和 `scripts/check-local.ps1 -RequireFeishu`，原模拟环境使用不带这些参数的命令。刷新页面后，从左侧“本机设置”查看开关；升级本身不写入开关选择。恢复旧版本前应先停止新版并恢复对应冷备份；旧版本不读取新界面保存的设置。
