...
5. 已持久化成功状态或同机有效租约下，同一 `eventId` 的正常重放不得创建第二个 Taskboard 任务；整体投递语义是至少一次，当前 Taskboard 缺少原生原子幂等键，因此不宣称绝对 exactly-once。
...
第一版继续使用现有 JSON 状态文件和本机 OS IPC 锁，避免引入新的数据库依赖。`stateFile` 必须是稳定的普通文件路径，不支持状态文件本身的符号链接、硬链接或多链接别名；锁只覆盖同一台 Windows/Linux 主机。存储接口需要扩展为原子领取和状态更新；后续事件量明显增长时，再单独评估 SQLite 迁移。
...
每次创建 Taskboard 任务前，继续使用服务端生成的事件元数据查找已有任务。即使请求已到达 Taskboard 但响应丢失，重试也会先发现已有任务再复用；但“查找”和“创建”不是 Taskboard 原子操作，若进程在 POST 成功后、状态提交或查询可见前崩溃，仍可能出现重复任务。
...
验收标准：暂停 Taskboard 后发送一条匹配事件，恢复 Taskboard，正常演练最终只产生一张任务并可由事件元数据核对；重启 Bridge 后未完成事件继续处理；真实长连接由 SDK 自动重连且已持久化成功的重复事件不会产生重复任务。验收不得把结果表述为跨崩溃窗口的绝对 exactly-once。
