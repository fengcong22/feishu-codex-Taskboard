...
export function assertPhasedSubjectMetadata(subject, metadata) {
  ...
  for (const [stageId, stage] of enabled) {
    ...
    if (seen.has(trigger.optionId)) throw configurationChanged("enabled stage trigger options must be unique");
    seen.add(trigger.optionId);
    const matches = options.filter((option) => option.id === trigger.optionId && option.name === trigger.value);
    if (matches.length !== 1) throw configurationChanged(`${stageId}.trigger option is missing or changed`);
  }
  return true;
}
