...
The same settings show the read-only machine automatic-execution policy from `GET /api/meta` → `capabilities.automaticExecution`. Older servers that omit it display an unknown state. The machine policy still defaults off and requires explicit deployment opt-in through `CODEX_TASKBOARD_ALLOW_AUTOMATIC_EXECUTION`; the subject selector cannot change it. Automatic starts additionally require trusted Bridge registration, an enabled automatic snapshot and an enabled local package. Simulated tasks remain ineligible.
