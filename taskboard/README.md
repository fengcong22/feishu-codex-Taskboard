...
The sidebar **Local settings → Allow automatic editing on this device** provides a persistent, immediately effective machine switch; subject settings link to the same dialog. `GET/PUT /api/local/settings/automatic-execution` returns `{setting:{enabled,version}}`; PUT requires `{enabled,expectedVersion}`, JSON, an explicit web client header and loopback peer/Host/Origin. Concurrent writes return `VERSION_CONFLICT`. The setting is stored in the local `taskboard_settings` table; `CODEX_TASKBOARD_ALLOW_AUTOMATIC_EXECUTION` supplies only the initial value before a setting is saved. The default remains off. Disabling cancels unstarted automatic reservations and returns queued tasks to `todo`, while running and manual executions continue. Enabling does not scan or launch historical tasks. `automatic-execution.updated` synchronizes open windows, and `/api/meta` reports the current effective value. Invalid stored settings fail closed and cannot be silently overwritten. The subject selector cannot change the machine switch. Automatic starts additionally require trusted Bridge registration, an enabled automatic snapshot and an enabled local package. Simulated tasks remain ineligible.
