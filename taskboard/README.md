...
The project Codex panel resolves a Feishu subject's saved fixed `packageRoute.packageAlias` through the enabled local package registry. Its `feishu-...` project ID need not match the package project ID. Catalogs, composer requests, and conversation turns use the same workspace resolver; missing or inactive packages and unavailable directories produce explicit errors without selecting another directory. Draft or paused subjects may still use an enabled package for manual chat. Create a new conversation after changing the bound workspace. Registered task conversations keep their package snapshots; ordinary tasks do not inherit the subject package workspace, automatic execution eligibility, or the task-only non-Git exception.
...
Each subject draft selects one Base single-select status field and uses that field's current options to configure the three fixed stages: initial cut, first-review revision, and final-review revision. Every stage has its own enable switch, trigger option, video source, review source, audio source, ZIP destination, and name suffix; at least one stage must remain enabled. Only an edge from another option into that stage's configured option registers a task. A disabled stage does not backfill missed work, and every registered task keeps an immutable copy of its stage and subject configuration.

The **audio source** editor is the same in `initial`, `first_review`, and `final_review`, but each stage saves its own value. Its single dropdown offers `Video original`, `Document section`, and `Base attachment field`. One shared detail control changes with the choice: video original shows a disabled control and a disabled tolerance input; document section shows an editable **Audio section title**; Base attachment field shows an **Audio attachment field** dropdown. The two replacement choices use one shared positive duration tolerance, defaulting to 3 seconds. The attachment dropdown contains only attachment-type fields from the current subject table. If none exist, `Base attachment field (no available fields)` is disabled; if a saved field disappears or changes type, save and enable fail closed instead of silently choosing another field. While editing, unsaved section titles, attachment selections, and the shared tolerance remain isolated to the current subject and stage. On the wire, these three choices remain the two existing modes: `video_original`, or `replace_original` with `docx_section`/`base_attachment`; no third audio mode is introduced.

A stage source is either one Base attachment field or a section in the single Feishu document link held by the configured document field. That link may be a direct `/docx/` URL or a `/wiki/` document URL. Document section anchors match trimmed heading text exactly and include nested content until the next heading at the same or higher level. Attachments retain source order and original filenames; multiple videos and replacement audios pair in document order, while a Base attachment source currently requires exactly one matching file. Missing headings, empty ranges, unreadable documents, download failures, count mismatches, or unreasonable duration differences block the run instead of guessing. `video_original` keeps each video's own audio; `replace_original` requires an explicit audio source and delegates muting/alignment to Auto-Cut Lite. The next Auto-Cut Lite candidate is required to add a controlled automatic-numbering fallback: exact heading text is attempted first; only after that fails may one supported leading number be removed from both sides, with no substring or fuzzy matching.

For the complete handoff contract, including the Auto-Cut Lite delivery checklist and acceptance matrix, see [the Auto-Cut Lite interface requirements](docs/auto-cut-lite-interface-requirements.md).

Each subject also stores manual or automatic execution mode, one fixed package alias, concurrency limits, upload policy, one naming field, and stage-specific ZIP destinations. The naming field may be plain text or a computed text result; Taskboard appends the frozen stage suffix and applies Auto-Cut Lite's Windows-safe 180-character name normalization before creating the run paths. Enabled stages in automatic mode require their own absolute local/UNC destination. Enabling performs live metadata and local package validation before replacing the active Bridge snapshot. Exported shared configuration omits credentials, workspace paths, ZIP source paths, upload paths, runtime state, and task history. Import first shows live Bridge plus local binding diagnostics, lists the actual warning/error messages for confirmation, and always writes subjects as drafts. Imported portable metadata and refresh timestamps never become locally verified state: existing subjects retain their Taskboard field snapshots, while new subjects start with a known-empty snapshot. Warned stale attachment bindings therefore remain blocked on Save and Enable until metadata is refreshed or the binding is repaired. The Taskboard-to-Bridge inspection request uses an explicit Bridge DTO and omits Taskboard-only project identities and cached metadata. Live phased validation also requires matching Base/table names, one unambiguous single-select status field, unique field/option IDs, and consistent field type metadata.
The workflow panel places **通用执行设置 (Shared execution settings) → 剪辑模式 (Editing mode)** above the three stage editors. Each subject has one manual/automatic selector shared by all stages, alongside its concurrency group, limit, and resource groups. Newly discovered subjects from Base metadata default to automatic drafts; configure their required fields, package and destinations, then use **Save draft → Enable** to publish. Existing subjects also show the selector and retain their saved modes through metadata refresh and removal/restoration. Historical configuration repair and shared imports with a missing mode keep the manual fallback. Saving preserves the active snapshot until explicit enable; changing the mode does not convert or rerun existing tasks. Upload enqueue mode remains independent.

The top-right header **Local settings → Allow automatic editing on this device** provides a persistent, immediately effective machine switch that remains accessible in narrow windows; subject settings link to the same dialog. `GET/PUT /api/local/settings/automatic-execution` returns `{setting:{enabled,version}}`; PUT requires `{enabled,expectedVersion}`, JSON, an explicit web client header and loopback peer/Host/Origin. Concurrent writes return `VERSION_CONFLICT`. The setting is stored in the local `taskboard_settings` table; `CODEX_TASKBOARD_ALLOW_AUTOMATIC_EXECUTION` supplies only the initial value before a setting is saved. The default remains off. Disabling cancels unstarted automatic reservations and returns queued tasks to `todo`, while running and manual executions continue. Enabling does not scan or launch historical tasks. `automatic-execution.updated` synchronizes open windows, and `/api/meta` reports the current effective value. Invalid stored settings fail closed and cannot be silently overwritten. The subject selector cannot change the machine switch. Automatic starts additionally require trusted Bridge registration, an enabled automatic snapshot and an enabled local package. Simulated tasks remain ineligible.

Refreshing metadata for an already-enabled subject creates a new local draft and leaves the last validated Bridge snapshot active until that draft is explicitly enabled. If a stable field or option ID has only been renamed, the panel shows the current name while preserving unsaved operator-entered source titles, attachment choices, tolerances, and suffixes; the operator must save the repaired draft before enabling it. A missing or duplicate option ID blocks both save and enable instead of being guessed or silently replaced.

Automatic execution requires the explicit machine opt-in described above; server `allowAutomaticExecution` or the environment is only the unsaved default. Startup rechecks the saved setting after asynchronous preparation, so turning off and back on cannot revive an already canceled start. Canonical registrations use each non-empty Feishu `event_id` as a global database-backed idempotency key, and the migrated schema claims the same key for inserts issued by the preceding Taskboard writer shape: an exact replay returns the existing task, while any Base, table, record, field, stage, delivery source, occurrence time, binding, or controlled-context mismatch is rejected as a conflict. An eligible canonical registration is acknowledged only after its durable execution reservation exists; an idempotent replay repairs a task whose creation committed before that reservation, even if the mutable subject or package is later disabled. Delayed wake-up, resource-queue launch, and restart recovery each revalidate the current trusted database origin against the task marker and `feishu` label, then require a production delivery and the current automatic mode. Revoked eligibility cancels the durable reservation and returns a queued task to `todo` instead of starting it. Pump and launch retry counters plus their exponential `readyAt` deadlines are durable, so recovery waits for the persisted backoff window; a wake-up can expedite an already-failed reservation only after current provenance is readable and never resets its retry budget. Archiving a Feishu task immediately cancels any delayed or queued execution. Unknown internal HTTP failures are logged only as a fixed safe error code, without exception messages or stacks. A successful Auto-Cut run remains `in_progress` until its Jianying ZIP is validated and SHA-256 hashed; that verified ZIP moves manual tasks to `in_review` and automatic tasks to `done`. Automatic upload configuration queues automatic tasks after ZIP verification and queues manual tasks after acceptance moves them to `done`; manual upload configuration leaves enqueueing to the completed-task UI. Upload jobs copy the verified artifact to a configured local or UNC/NAS destination with per-subject concurrency, manual retry, and conflict protection.

The workflow panel supports `manual_select` and `driver_report`; `watch_directory` remains reserved and disabled. For `driver_report`, configure an absolute ZIP source root. Eligible automatic phased starts, including delayed reservations recovered after restart, use Taskboard's local Auto-Cut runner. The runner also handles explicit phased retries carrying both consent fields below. Taskboard resolves the registered deployment's fixed Python and workflow entry point, passes only the run-owned manifest, result-receipt and expected ZIP paths, then hashes that exact ZIP and submits the existing run-scoped artifact report internally. This path does not launch a Codex turn or require `taskctl` on PATH. Manual starts and consent-free retries retain the Codex path; its artifact-report instruction uses the server-provided absolute Node and Taskboard CLI entry points.

Configure local runner deadlines in the Taskboard service environment before starting it. `CODEX_TASKBOARD_AUTOCUT_ENVIRONMENT_TIMEOUT_MS` bounds the CLI/directory probe (default `30000`), `CODEX_TASKBOARD_AUTOCUT_PREFLIGHT_TIMEOUT_MS` bounds preflight (default `120000`), and `CODEX_TASKBOARD_AUTOCUT_RUN_TIMEOUT_MS` bounds the full editing process (default `7200000`). Values must be positive integer milliseconds no greater than `2147483647`; an invalid configured value blocks the run with `AUTOCUT_TIMEOUT_INVALID`. Taskboard snapshots these numeric options at service startup and passes them directly to its runner. It strips all `CODEX_TASKBOARD_*` variables and Bridge/application credentials from the child environment. Changing a deadline requires restarting Taskboard after active work has finished; it does not retry an existing blocked or completed task.

Local artifact registration has a five-minute response deadline, including reading the response body. HTTP failures, connection failures, timeouts and malformed success responses block the attempt with safe error codes (`AUTOCUT_ARTIFACT_REPORT_FAILED`, `AUTOCUT_ARTIFACT_REPORT_TIMEOUT`, or `AUTOCUT_ARTIFACT_REPORT_INVALID`) and release its execution claim. A success response must agree with the stored, verified artifact and immutable run binding before completion. Graceful shutdown keeps the loopback listener open while an already-prepared result is registered, so an unresponsive registration can delay shutdown until its deadline. A blocked attempt remains available for diagnosis and explicit retry; it is never treated as a successful edit or silently rerun.

Before editing, a bounded subprocess checks the installed deployment report's absolute CLI path, existing strict user identity via `whoami`, readiness file access and temporary-file creation in the readiness/job/drafts/ZIP parent directories. On Windows, the supported deployment layout is an npm shim (`.cmd`, `.ps1`, or `.bat`) alongside `lark-cli.cmd` and `node_modules/@larksuite/cli/scripts/run.js`. If an adjacent `node.exe` exists, it must resolve to Taskboard's Node executable. Other Windows layouts fail closed because the installed Python adapter's PATH discovery could select a different CLI. The service account must retain access to its own CLI authentication directories and credential store; credentials are not copied or printed. Set `AUTOCUT_LITE_READINESS_PATH` to an absolute readiness file path when overriding the `%LOCALAPPDATA%/Auto-Cut/auto-cut-lite/runtime-readiness.json` default. The probe preserves existing readiness content. Failures return `AUTOCUT_LARK_CLI_UNAVAILABLE`, `AUTOCUT_LARK_IDENTITY_UNAVAILABLE`, `AUTOCUT_READINESS_UNAVAILABLE`, or `AUTOCUT_OUTPUT_UNAVAILABLE`; stalled probes/preflight/runs return the corresponding `AUTOCUT_*_TIMEOUT`. Preflight completion is read from Auto-cut-lite's stderr JSON progress stream.
...
`runConsent` is optional; omitting it preserves the existing Codex retry behavior. When present, both fields must be literal `true` booleans and no other fields are accepted. A retry carrying both fields for a server-registered, blocked, phased Feishu Auto-Cut task uses the local runner without starting another Codex turn. Automatic phased starts use that runner only after the independent machine switch, trusted production registration, active automatic snapshot and enabled local package checks; this does not imply consent for a later manual retry. Ordinary tasks, copied description markers, and legacy or unregistered tasks cannot obtain local execution capability. The explicit retry approval is held only in memory from retry scheduling through creation of that one run. The raw request/object is not persisted, inherited, copied into package prompts or ordinary user messages, or placed in environment variables; authorization is never taken from comments, Feishu cells, paths, or free text. The local runtime receives only the server-owned run paths and binding, may send video audio only to `openspeech.bytedance.com` for word-level timing and acceptance, and may write the Jianying draft/ZIP only to the configured `CODEX_AUTOCUT_DRAFTS_ROOT` and `CODEX_AUTOCUT_PACKAGE_ZIP_PATH` locations. Taskboard still uses the exact run-bound `driver_report` ZIP, never scans a directory or guesses the newest file, and does not write status or results back to Feishu. If the service restarts before an explicitly consented retry creates its run, that in-memory approval is lost and a new consented retry must be authorized again.
...
| `CODEX_TASKBOARD_ALLOW_AUTOMATIC_EXECUTION` | unset (off) | Initial opt-in (`1`, `true`, `yes`, `on`) only before a local setting is saved; the UI choice then takes precedence |
...
Feishu stage cards use the controlled naming-field snapshot in `subject · course name · stage` titles, with record ID as the blank-name fallback. Generated titles are shortened to the shared 240-character task API limit while the complete naming snapshot is retained. Historical generated titles are projected from their captured subject version and naming snapshot without rewriting provenance, task versions or custom titles. Missing historical names are not fetched from Feishu.
