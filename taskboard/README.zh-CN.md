...
飞书项目的 Codex 面板使用当前已保存学科配置的固定 `packageRoute.packageAlias` 解析本机已启用包的工作目录，不要求 `feishu-...` 项目 ID 与包的项目 ID 相同。模型列表、输入框和对话发送统一校验该绑定；包缺失、未启用或目录不存在时明确阻断，不回退到其他目录。草稿/暂停学科仍可手动使用已启用包；重新绑定目录后须新建对话。受控任务仍按其登记的包快照解析，普通任务不会继承该包目录或自动执行资格，项目级对话也不获得任务专用的非 Git 例外。

包列表的“引用”数量和删除检查共用后端规则。已归档且没有活动执行的任务不再阻止删除；未归档的未完成任务、待执行或执行中的预约、启动认领、运行中的 AI 实例，以及已启用工作流的活动配置仍会阻止删除，活动执行不会因任务归档而被忽略。编辑或刷新出新草稿时，原活动配置仍算引用；需改绑其他包并启用新配置，或显式停用该学科，才能解除配置占用。

删除包会保留历史任务、服务端来源登记、不可变配置快照及事件去重记录。恢复服务端登记的飞书任务时，原包别名必须仍存在且已启用；原包已删除、禁用或为草稿时，接口返回受控冲突并提示先配置、启用原包。恢复不会刷新历史快照或安排执行，普通任务或复制的描述标记也不会因此获得执行权限。不要通过清空历史任务、数据库或 Bridge 状态来解除包占用。
...
每个学科草稿独立保存唯一触发字段/值、手动或自动执行模式、Auto-Cut 包别名、并发限制和上传策略。点击启用前会实时校验飞书元数据和本机包配置，验证成功后才替换 Bridge 活动快照。导出的共享配置不会携带凭据、工作区路径、ZIP 获取路径、上传路径、运行状态或历史任务；导入会先合并 Bridge 实时诊断和 Taskboard 本机绑定诊断，在确认框列出实际警告/错误消息，并始终以草稿落库。导入文件携带的便携元数据和刷新时间不会成为本机已验证状态：已有学科保留 Taskboard 本机字段快照，新学科以明确的空字段快照开始；被警告的过期附件绑定在刷新元数据或修复字段前，仍会阻止保存和启用。Taskboard 请求 Bridge 检查时使用明确的 Bridge DTO，不发送 Taskboard 专用项目 ID 或缓存 metadata；三阶段实时校验还要求 Base/子表名称一致、状态字段是唯一且无歧义的单选字段、字段和选项 ID 不重复，并拒绝互相冲突的字段类型信息。

选中学科后，在三阶段配置上方的 **通用执行设置 → 剪辑模式** 切换“自动 / 手动”；初稿、初审修改、终审修改共用这一项，并发组、并发数和资源组也集中在这里。首次从 Base 元数据发现的新学科默认是自动草稿，补齐必要配置并点击“保存草稿 → 启用”后才生效。现有学科同样有开关，保留已保存的模式，刷新或移除后恢复不会改为自动；历史配置修复和共享配置缺少模式时仍回退手动。修改只影响启用后新触发的任务，保存草稿期间原活动配置继续有效；已有任务不会被改模式或自动重跑。“上传入队”继续独立设置。

通用执行设置还显示只读的本机自动执行总开关状态，来自 `GET /api/meta` 的 `capabilities.automaticExecution`。旧服务未提供该字段时显示“状态未知”。总开关关闭时，学科选自动也不会自动启动；仍需显式开启本机部署策略，并满足可信 Bridge 来源、已启用配置及本机包白名单等条件。页面不能更改本机总开关，模拟任务不能自动执行。
“音频来源”编辑器在 `initial`（初稿）、`first_review`（初审修改）和 `final_review`（终审修改）三个阶段使用相同界面，但每个阶段独立保存。下拉菜单固定提供“视频原音”“文档目录”“Base 字段附件”三个选项。下方只保留一个公用详情入口：选择视频原音时详情和时长误差输入框可见但置灰；选择文档目录时显示可手动输入的“音频目录标题”；选择 Base 字段附件时显示“音频附件字段”下拉。两种替换来源共用一个正数时长误差，默认 3 秒。附件字段下拉只显示当前学科表中类型为附件的字段；没有附件时显示置灰的“Base 字段附件（无可用字段）”。已保存字段在元数据刷新后失效或类型改变时，保存和启用都会 fail-closed，不会静默换字段。未保存编辑期间，各阶段各学科的目录标题、附件字段和共用误差值会保留，但只持久化当前选中的来源。界面三种选择在接口上仍只对应两个既有模式：`video_original`，或带 `docx_section`/`base_attachment` 的 `replace_original`，不新增第三种音频模式。下一版 Auto-Cut Lite 候选版本必须新增受控的自动编号兼容：先精确匹配文档目录，精确失败后才允许从双方开头各去掉一个受控编号再完整匹配，不允许子串或模糊匹配。

完整的 Auto-Cut Lite 接口、交付清单和验收矩阵见[统一接口需求文档](docs/auto-cut-lite-interface-requirements.md)。
...
三阶段工作流只在各阶段设置 **ZIP 目标目录**，自动上传启用时检查每个已启用阶段的目录，关闭阶段可以留空。“ZIP 与上传”不再显示公共上传路径和上传目标别名；保留 ZIP 获取方式、ZIP 来源根目录、上传入队和上传并发数。来源根目录决定从哪里获取生成的 ZIP，阶段目标目录决定上传到哪里。旧版工作流仍可设置公共上传路径；已有配置中的公共路径和可选别名、历史上传记录继续兼容，已有三阶段任务始终使用创建时保存的阶段目录。
