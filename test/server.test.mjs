...
test("includes listener state when a health provider is configured", async (t) => {
  const app = await start(assert.fail, () => ({ ok: true, feishuListener: "connected" }));
  t.after(app.close);
  const health = await (await fetch(`${app.url}/health`)).json();
  assert.deepEqual(health, { ok: true, feishuListener: "connected" });
});
